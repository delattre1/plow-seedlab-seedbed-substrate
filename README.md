...
> **New engineer? Start with [`docs/AUTH-BANK-ONBOARDING.md`](docs/AUTH-BANK-ONBOARDING.md)** — the onboarding walkthrough (what it is, the browserauth login flow, leasing, the canonical manifest + lease dir, the status dashboard, and what NOT to do).
...
- **Auth bank is live state.** The bank is 10 **pre-authed** Claude volumes on the docker host. A brand-new machine has none — they require per-volume OAuth device logins (browserauth). `bank/volumes.txt` lists names only; the authed volumes themselves are host state. Provisioning *reuses* these volumes (it does **not** do a fresh login per spin).
  - **CURRENT STATE (2026-07-01) — bank consolidation DONE:** the fresh canonical bank `claude-auth-bank-01` … `claude-auth-bank-10` is **10/10 AUTH_OK** (probed concurrently), authed via the proven browser flow (`daniel@plow.co`, browserauth CDP). Canonical manifest `bank/volumes.txt` lists exactly these 10 (verified); canonical lease dir `~/.config/seedbed/leases-bank`; workstream configs (`bridge.env` + `core.env`) point to the ONE manifest + ONE lease dir. Live `/hydrate` on the canonical bank = **verdict PASS (8/8)**. Phase 4 retired 54 old auth volumes. **Straggler note (verified on host 2026-07-01):** just **1** non-canonical `claude-auth-*` volume remains pending drain — `claude-auth-discordhydrate-join-1`, held by the frozen `core-mp-13709-1` (CEO stand-down); it retires on the core-node release → converges to exactly the 10 canonical. (`claude-auth-helloworld-3` + `-live` were retired. A transient `claude-auth-demo11` exists for the explainer video only — being torn down post-capture, **NOT a bank member**.) Everything else is consolidated to the canonical 10.
