...
# never let an empty pre-existing "Boss" window block the spawn (golden-boot leaves a
# neutral "console" placeholder, so killing a stray Boss can't kill the session)
$DOCKER exec "$N" bash -lc 'tmux kill-window -t mc-main:Boss 2>/dev/null || true' 2>/dev/null || true

"$MP" spawn "$N/main:Boss" --cwd /home/tester --backend claude --boss "$CENTRAL_BOSS"

# wait for the Boss claude to render, then kill ALL other windows so only "Boss" remains
for _ in $(seq 1 30); do
  $DOCKER exec "$N" bash -lc 'tmux capture-pane -t mc-main:Boss -p 2>/dev/null | grep -q "bypass permissions on"' && break
  sleep 1
done
$DOCKER exec "$N" bash -lc 'tmux kill-window -a -t mc-main:Boss 2>/dev/null || true' 2>/dev/null || true
